...
module "redshift_schema_lambda" {
  ...
  lambda_base_dir             = "${abspath(path.cwd)}/../../../etl/lambdas/redshift_schema"
  lambda_source_dir           = "${abspath(path.cwd)}/../../../etl/lambdas/redshift_schema/src"
  memory_size                 = 1000
  timeout                     = 900
...
resource "aws_kms_grant" "schema_lambda_grant_kms_key" {
  ...
  grantee_principal = module.redshift_schema_lambda.aws_lambda_function_role_arn
  operations = [
    "Decrypt",
    "Encrypt",
    "GenerateDataKey",
...
resource "aws_s3_object" "redshift_schema_table_user" {
  ...
  key            = "users.sql"
  content_base64 = filebase64("${path.module}/../../../etl/lambdas/redshift_schema/schemas/users.sql")
}

########################################################################################################################
###   Allow Bucket to Notification to AWS Lambda
########################################################################################################################

resource "aws_lambda_permission" "allow_bucket" {
  statement_id  = "AllowExecutionFromS3Bucket"
  action        = "lambda:InvokeFunction"
  function_name = module.redshift_schema_lambda.aws_lambda_function_arn
...
resource "aws_s3_bucket_notification" "bucket_notification" {
  ...
  lambda_function {
    lambda_function_arn = module.redshift_schema_lambda.aws_lambda_function_arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = ""
    filter_suffix       = ".sql" # Ensure this matches the SQL files' extension
  }
